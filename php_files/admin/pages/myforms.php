...
<?php require("header.php"); ?>
      ...
      $sql = "SELECT ypallhlosid FROM ypallhlos WHERE username=:username";
      $statement = $pdoObject -> prepare($sql);
      $statement->execute( array(':username'=>$_SESSION['username']));
      if ($record = $statement -> fetch()) {
          $ypid = $record['ypallhlosid'];     
      } 
      $sql2 = "SELECT * FROM adeies INNER JOIN typos_adeias ON adeies.typos_id=typos_adeias.typos_id INNER JOIN katastash ON adeies.katastasi_id=katastash.katastasi_id WHERE ypallhlosid=:ypallhlosid";
      $statement = $pdoObject -> prepare($sql2);
      $statement->execute( array(':ypallhlosid'=>$ypid));
